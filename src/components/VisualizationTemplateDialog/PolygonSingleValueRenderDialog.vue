<template>
  <el-dialog v-model="isDialogShow" title="面矢量单值制图" width="20%" center @close="closeThisDialog">
...
const emit = defineEmits(['closePolygonSingleValueRenderDialog', 'confirmPolygonSingleValueRenderDialog'])
...
const closeThisDialog = () => {
  emit('closePolygonSingleValueRenderDialog')
}
const polygonSingleValueRender = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      emit('closePolygonSingleValueRenderDialog')
      emit('confirmPolygonSingleValueRenderDialog', formData.data)
    }
  })
}
