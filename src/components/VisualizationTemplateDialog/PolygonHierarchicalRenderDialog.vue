<template>
  <el-dialog v-model="isPolygonHierarchicalRenderDialogShow" title="面矢量分级制图" width="20%" center @close="closeThisDialog">
...
const props = defineProps({
  isPolygonHierarchicalRenderDialogShow: {
    type: Boolean,
    default: false
  },
  // layerList: {
...
const emit = defineEmits(['closePolygonHierarchicalRenderDialog', 'confirmPolygonHierarchicalRenderDialog'])
const { isPolygonHierarchicalRenderDialogShow } = toRefs(props)
...
watch(isPolygonHierarchicalRenderDialogShow, (val) => {
  if (val) {
...
const closeThisDialog = () => {
  emit('closePolygonHierarchicalRenderDialog')
}
const polygonUniqueValueRender = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      ...
      emit('closePolygonHierarchicalRenderDialog')
      emit('confirmPolygonHierarchicalRenderDialog', formData.data)
    }
  })
}
