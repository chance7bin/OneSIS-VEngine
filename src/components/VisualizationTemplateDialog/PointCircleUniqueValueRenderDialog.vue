<template>
  <el-dialog v-model="isPointUniqueValueRenderDialogShow" title="点矢量唯一值制图" width="20%" center @close="closeThisDialog">
...
const props = defineProps({
  isPointUniqueValueRenderDialogShow: {
    type: Boolean,
    default: false
  },
  // layerList: { ·
...
const emit = defineEmits(['closePointUniqueValueRenderDialog', 'confirmPointUniqueValueRenderDialog'])
const { isPointUniqueValueRenderDialogShow } = toRefs(props)
...
watch(isPointUniqueValueRenderDialogShow, (val) => {
  if (val) {
...
const closeThisDialog = () => {
  emit('closePointUniqueValueRenderDialog')
}
const polygonUniqueValueRender = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      ...
      emit('closePointUniqueValueRenderDialog')
      emit('confirmPointUniqueValueRenderDialog', formData.data)
    }
  })
}
