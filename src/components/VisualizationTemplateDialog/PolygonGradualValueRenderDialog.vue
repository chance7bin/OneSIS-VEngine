<template>
  <el-dialog v-model="isPolygonGradualValueRenderDialogShow" title="面矢量渐变颜色制图" width="20%" center
...
const props = defineProps({
  isPolygonGradualValueRenderDialogShow: {
    type: Boolean,
    default: false
  },
  // layerList: {
...
const emit = defineEmits(['closePolygonGradualValueRenderDialog', 'confirmPolygonGradualValueRenderDialog'])
const { isPolygonGradualValueRenderDialogShow } = toRefs(props)
...
watch(isPolygonGradualValueRenderDialogShow, (val) => {
  if (val) {
...
const closeThisDialog = () => {
  emit('closePolygonGradualValueRenderDialog')
}
const polygonUniqueValueRender = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      ...
      emit('closePolygonGradualValueRenderDialog')
      emit('confirmPolygonGradualValueRenderDialog', formData.data)
    }
  })
}
