<template>
  <el-dialog v-model="isPolylineGradualValueRenderDialogShow" title="线矢量渐变颜色制图" width="20%" center
...
const props = defineProps({
  isPolylineGradualValueRenderDialogShow: {
    type: Boolean,
    default: false
  },
  // layerList: {
...
const emit = defineEmits(['closePolylineGradualValueRenderDialog', 'confirmPolylineGradualValueRenderDialog'])
const { isPolylineGradualValueRenderDialogShow } = toRefs(props)
...
const uniqueValuesASC = ref<Array<any>>([])
watch(isPolylineGradualValueRenderDialogShow, (val) => {
  if (val) {
...
const closeThisDialog = () => {
  emit('closePolylineGradualValueRenderDialog')
}
const polygonSingleValueRender = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      emit('closePolylineGradualValueRenderDialog')
      emit('confirmPolylineGradualValueRenderDialog', formData.data)
    }
  })
}
...
const handleFieldSelectChange = (val: any) => {
  geoFileApi.getUniqueValues(formData.data.layer, val, formData.data.sort).then((res: any) => {
    if (res.code === 200) {
      console.log(res.data)
